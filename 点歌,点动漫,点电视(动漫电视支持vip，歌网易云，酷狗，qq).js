...
async lbcd(e){
 ...
 async shareMusic(e) {
  ...
  try {
    ...
    if (e.isPrivate) {
	  ...
	  if(isQQ &id == ""){
		  for(let i=0;i<data.length;i++){
				  ...
				  msg = msg+ String(i+1)+ ".  "+data[i]. song + "   作者："+ data[i]. singers + ","
	...
	data1 = {
  tplFile: './plugins/example/html2/2.html',
	...
	e.reply(img)
	  ...
	  if(isKugou &id == ""){
		  ...
		  for(let i=0;i<data. info.length;i++){
				...
				msg =  msg+ String(i+1)+ ".  "+data. info[i].songname + "   作者："+ data. info[i].singername + ","
				  console.log(msg)
	  }
	...
	e.reply(img)
      ...
      if (isWangYiyun &id == "") {
			   ...
			   for(let i=0;i<songList.length;i++){
				  ...
				  msg = msg+ String(i+1)+ ". "+songList[i].name + " 作者："+ songList[i].artists[0].name + ","
