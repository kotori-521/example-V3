    ...
    async accept(e) {
        ...
        switch (e.sub_type) {
            ...
            case `recall`: {
                ...
                if (!config.MasterRecall) return;

                let pathMaster = `./resources/message/Master/${e.user_id}.json`

                let res = getread(pathMaster)
...
export class anotice extends plugin {
    constructor() {
        ...
        this.Master = './resources/message/Master'
    ...
    async init() {
        ...
        if (!fs.existsSync(this.Master)) {
            fs.mkdirSync(this.Master)
        }
    }
    ...
    async accept(e) {
        ...
        if (e.isGroup) {
            ...
            if (!data_redis) {
                getwrite(this.paths, {})
                redis.set(key, "1", { EX: config.deltime })
            }

            getobtain(this.paths, message_id, message)

        } else {
            if (!config.MasterRecall) return;
            this.paths = `${this.Master}/${e.user_id}.json`
            let key = `YZ:groupmessagedel:${e.user_id}`
            let data_redis = await redis.get(key);
...
//读取文件
function getread(path) {

    let arr = fs.readFileSync(path, "utf8", (err, data) => {
        if (err) {
            console.log(err)
            return "error";
        }
...
//写入文件
function getwrite(path, cot) {

    fs.writeFileSync(path, JSON.stringify(cot, "", "\t"), "utf-8",
        err => { console.log('写入成功', err) })

}

function getobtain(path, message_id, message) {
    if (!fs.existsSync(path)) {
        getwrite(path, { [message_id]: message })
        return;
    }
    let arr = getread(path)
