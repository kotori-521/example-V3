//by 癫癫博士
//全局表情添加，添加表情后全部的群均可使用
//比较双刃剑，若被滥用可能极大影响群聊，我还没想到平衡的办法，故此文件为测试中
//此文件由V3的yunzai添加表情功能更改得来
...
export class all_add extends plugin {
  constructor () {
    super({
      name: '全局添加表情',
      dsc: '全局添加表情，文字等',
  ...
  async accept () {
    /** 处理消息 */
    if (this.e.atBot && this.e.msg && this.e?.msg.includes('全局添加') && !this.e?.msg.includes('#')) {
  ...
  async add () {
    ...
    if (!this.group_id) {
      this.e.reply('请先在群内触发表情，确定全局添加的群')
      return
    }
    ...
    if (!this.keyWord) {
      this.e.reply('全局添加错误：没有关键词')
      return
    }

    this.setContext('addContext')

    await this.e.reply('请发送全局添加内容', false, { at: true })
  }
  ...
  async getGroupId () {
    if (this.e.isGroup) {
      ...
      redis.setEx(this.grpKey, 3600 * 24 * 30, String("全局"))
  ...
  checkKeyWord () {
    if (this.e.img && this.e.img.length > 1) {
      this.e.reply('全局添加错误：只能发送一个表情当关键词')
      return false
    }
    ...
    if (this.e.at) {
      ...
      if (at.length > 1) {
        this.e.reply('全局添加错误：只能@一个人当关键词')
        return false
      }
    }
  ...
  async singleAdd () {
    ...
    let keyWord = this.e.msg.replace(/#|＃|图片|全局表情|全局添加/g, '').trim()
    if (!keyWord) return false

    this.keyWord = this.trimAlias(keyWord)
    this.e.keyWord = this.keyWord

    if (this.e.msg.includes('全局添加图片')) {
      this.e.addImg = true
    }
    ...
    for (let i in message) {
      if (message[i].type == 'at') {
        if (message[i].qq == Bot.uin) {
          this.e.reply('全局添加内容不能@机器人！')
          return
        }
      }
      if (message[i].type == 'file') {
        this.e.reply('全局添加错误：禁止全局添加文件')
        return
      }
    }
  ...
  getRetMsg () {
    ...
    if (retMsg?.addContext?.message) {
      ...
      for (let i in msg) {
        if (msg[i].type == 'text' && msg[i].text.includes('全局添加')) {
          msg[i].text = this.trimAlias(msg[i].text)
          msg[i].text = msg[i].text.trim().replace(/#|＃|图片|全局表情|全局添加/g, '')
  ...
  saveJson () {
    ...
    fs.writeFileSync(`${this.path}全局.json`, JSON.stringify(obj, '', '\t'))
  }

  async saveImg (url, keyWord) {
    let groupCfg = cfg.getGroup(this.group_id)
    let savePath = `${this.facePath}全局/`
  ...
  async getText () {
    ...
    if (isNaN(keyWord)) {
      ...
      if (!isNaN(num) && !textArr["全局"].has(keyWord)) {
    ...
    logger.mark(`[发送全局表情]${this.e.logText} ${keyWord}`)
  ...
  expiredMsg (keyWord, num) {
    logger.mark(`[发送全局表情]${this.e.logText} ${keyWord} 全局表情已过期失效`)
  ...
  initTextArr () {
    ...
    /** 加载表情 */
    let facePath = `${this.facePath}全局`

    if (fs.existsSync(facePath)) {
      const files = fs.readdirSync(`${this.facePath}全局`).filter(file => /\.(jpeg|jpg|png|gif)$/g.test(file))
  ...
  async del () {
    ...
    let keyWord = this.e.toString().replace(/#|＃|图片|全局表情|全局删除|全部/g, '')
    ...
    let retMsg = [{ type: 'text', text: '全局删除成功：' }]
    for (let msg of this.e.message) {
      if (msg.type == 'text') {
        msg.text = msg.text.replace(/#|＃|图片|全局表情|全局删除|全部/g, '')
  ...
  async list () {
    ...
    let search = this.e.msg.replace(/#|＃|全局表情|词条/g, '')
    ...
    if (type == 'list' && count > 100) {
      end = `更多内容请翻页查看\n如：#全局表情列表${Number(page) + 1}`
    }

    let title = `全局表情列表，第${page}页，共${count}条`
    if (type == 'search') {
      title = `全局表情${search}，${count}条`
    }
